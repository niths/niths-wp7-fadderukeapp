    ...
    restClient.update(
        ...
        function(data, textStatus, jqXHR) {
          ...
          if(textStatus == "success"){
            ...
            showMsg('Oppdatering vellykket', null);
          } else {
            showErr("Oppdatering feilet", null);
          }
        }
    );
...
function printUserInfo() {
  ...
  $('#profileImg').attr(
      'src',
      'http://www.gravatar.com/avatar/' + userEmail + '?d=mm');

  if(student.fadderGroup != null) {
    $('#faddergroup').html(
        '<a class="blacklink" href="#single-fadder-group-page?group-id=' +
          student.fadderGroup.id+'">' + student.fadderGroup.groupNumber +
        '</a>');
  } else {
    $('#faddergroup').html(
        '<a class="blacklink" href="#scan-qr-page">Trykk her</a>');
  }
