$("#program-page").live('pageinit', function() {
  ...
  $("#privacyselectdiv input[type='radio']").bind("change",
      function(event, ui) {
  ...
  function getUrlParam(isPrivate){
    ...
    var param = '';
    if(isPrivate){
      param = '?tag=fadderuka12,gruppe'+student.fadderGroup.groupNumber +
          '&startTime='+today + '&endTime=' + inFiveDays;
  ...
  function getHeader(date){
    return '<h1 class="withborder">' + getDayName(date.getDay()) +
        ' '+date.getDate()+'/'+(date.getMonth()+1)+'</h1>';
  }
  ...
  function getNewList(name){
    return '<ul id="' + name +
        '" data-role="listview" class="ui-listview" data-inset="true">';
  }
  ...
  function handleData(data){
    ...
    for(var i=0;i<data.length;i++){
      ...
      if(i == 0){
        currentEventDate.setFullYear(
            data[i].startTime.substring(6,10),
            (data[i].startTime.substring(3,5)-1),
            data[i].startTime.substring(0,2)
        );

        html = getHeader(currentEventDate) + getNewList('list' + current);
      }
        //Set the temp date to the object were iterating over 
      tempEventDate.setFullYear(
          data[i].startTime.substring(6,10),
          (data[i].startTime.substring(3,5)),
          data[i].startTime.substring(0,2)
      );
      ...
      if(currentEventDate.getDate() != tempEventDate.getDate()) {
        ...
        current++;
        currentEventDate.setFullYear(
            tempEventDate.getFullYear(),
            (tempEventDate.getMonth()-1),
            tempEventDate.getDate()
        );
