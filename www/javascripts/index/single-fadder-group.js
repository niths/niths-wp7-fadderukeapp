      ...
      if (attribute == 'leaders') {
        if(fadderGroup[attribute].length < 1) {
          $('#groupleaderlist').html(
              '<li class="li-first"><h3>Ingen ledere registert</h3></li>');
        } else {
        ...
        if(fadderGroup[attribute].length < 1){
          $('#groupmemberslist').html(
              '<li class="li-first"><h3>' +
              'Ingen fadderbarn registert</h3></li>');
        } else {
...
function traverseFadderLeaderAttributes(fadderLeaders) {
  $('#groupleaderlist').html('');
    $.each(fadderLeaders, function(i, fadderLeader) {
      $('#groupleaderlist').append(
          '<li class="li-first">'+
          '<img src="http://www.gravatar.com/avatar/' +
            hex_md5(fadderLeader.email) + '?d=mm" />'+
          '<h3><strong>'+fadderLeader.firstName + ' ' +
            fadderLeader.lastName +'</strong></h3>'+
          '<p>Mail: '+fadderLeader.email+'</p><p>Tlf: ' +
            fadderLeader.telephoneNumber+'</p></li>');
    });
}
